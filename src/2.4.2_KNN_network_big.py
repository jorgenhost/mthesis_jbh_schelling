...
df_hh = lf_hh.collect(new_streaming=True)
...
df_out = geo.query_knn(kd_tree, coords=coords, df = df_hh, k=k).sort('hh_id')

df_out.select(pl.all().exclude("etrs89_east", "etrs89_north","etage", "sidedoer")).write_parquet(f'{DATA_DIR}/build/knn{k}_prep.pq')
df_out = None

time.sleep(10)

lf_knn = pl.scan_parquet(f'{DATA_DIR}/build/knn{k}_prep.pq').sort('hh_id')
lf_nn = lf_hh.select(pl.all().exclude("etrs89_east", "etrs89_north","etage", "sidedoer")).rename(lambda col_name: f'{col_name}_nn').collect(new_streaming=True).lazy()
